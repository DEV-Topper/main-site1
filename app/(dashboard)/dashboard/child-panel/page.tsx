...
import { Monitor, Globe, CheckCircle2, AlertCircle, ArrowRight, Loader2, ChevronDown, Copy, Check, Clock } from "lucide-react";
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <AnimatePresence mode="wait">
            ...
            {/* ─── EXISTING PANEL: PENDING OR ACTIVE ─── */}
            {existingPanel?.exists ? (
              <motion.div
                key="existing"
              ...
              >
                ...
                <div className={`bg-card border rounded-2xl p-6 md:p-8 shadow-sm flex items-start gap-5 ${
                  ...
                  <div className="space-y-1">
                    <h2 className="text-xl font-bold">
                      {existingPanel.status === 'active' ? 'Panel Active!' : existingPanel.status === 'rejected' ? 'Panel Rejected' : 'Panel Pending Approval'}
                    </h2>
                    <p className="text-muted-foreground text-sm">
                      {existingPanel.status === 'active'
                        ? `Your panel at ${existingPanel.domain} is live! Your users can visit your domain to recharge.`
                        : existingPanel.status === 'rejected'
                        ? 'Your panel request was rejected. Please contact support for more details.'
                        : `Your panel for ${existingPanel.domain} is being reviewed. DNS setup below ↓`}
                    </p>
                    <div className="flex items-center gap-3 pt-2 flex-wrap">
                      <span className={`text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider ${
                        existingPanel.status === 'active' ? 'bg-green-500/10 text-green-600' :
                        existingPanel.status === 'rejected' ? 'bg-red-500/10 text-red-600' :
                ...
                {/* DNS Setup Instructions — always show while pending/active */}
                {(existingPanel.status === 'pending' || existingPanel.status === 'active') && (
                  <div className="bg-slate-900 text-white rounded-2xl overflow-hidden shadow-2xl">
                    <div className="p-5 md:p-6 border-b border-white/10 flex items-center gap-3">
                      ...
                      {existingPanel.status === 'active' && (
                        <span className="ml-auto text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-green-500/20 text-green-400 border border-green-500/20">Verified</span>
                      )}
                    </div>
                    <div className="p-5 md:p-6 space-y-5">
                      <p className="text-sm text-slate-300">
                        Go to the place where you bought your domain (e.g. Namecheap, GoDaddy, Cloudflare) and add the following record:
                      </p>
                            ...
                            <tr className="border-t border-white/10">
                              ...
                              <td className="px-4 py-3 font-mono text-xs">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-slate-500 text-[10px]">(or www)</span>
                                </div>
                              </td>
                      ...
                      <div className="space-y-3">
                        <p className="text-xs font-bold text-slate-300 uppercase tracking-wider">Step-by-step:</p>
                        {[
                          { step: "1", text: "Log into your domain registrar (Namecheap, GoDaddy, Cloudflare, etc.)" },
                          { step: "2", text: `Find the DNS settings for ${existingPanel.domain}` },
                          { step: "3", text: "Add a new CNAME record with the name @ (or www) pointing to cname.vercel-dns.com" },
                          { step: "4", text: "Save the changes and wait up to 24 hours for DNS to propagate globally" },
                          { step: "5", text: "Once your panel is approved & DNS is set, your customers can visit your domain!" },
                        ].map(({ step, text }) => (
                          <div key={step} className="flex items-start gap-3">
                            <div className="w-6 h-6 rounded-full bg-blue-500/20 text-blue-400 text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">{step}</div>
                      ...
                      <div className="p-4 rounded-xl bg-yellow-500/10 border border-yellow-500/20 text-yellow-300 text-xs">
                        ⚠️ DNS changes can take 24-48 hours to propagate. Your domain will start working once both DNS and panel approval are complete.
                      </div>
                    </div>
                  </div>
                )}
              ...
              <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                ...
                >
                  ...
                  <div className="flex flex-col gap-3 pt-2">
                    ...
                    <button onClick={() => setShowBalanceModal(false)} className="w-full py-3 text-muted-foreground hover:text-foreground font-medium transition-colors">
                      Close
                    </button>
                  </div>
                </motion.div>
        ...
        {/* Right column */}
        <div className="space-y-6">
          {/* DNS accordion — show when no existing panel */}
          {!existingPanel?.exists && (
                  ...
                  >
                    <div className="p-5 pt-0 space-y-4 border-t border-border/50">
                      <p className="text-[13px] text-muted-foreground leading-relaxed">
                        After purchase, point your domain to:
                      </p>
